### Cad - Save as: convert image from storage to another format
```python
...
try:
    ...
    result = cad_api.upload_file(upload_file_request)
    ...
    converted_image = cad_api.save_image_as(save_as_request)
...
### Cad - Save as: convert image from request stream to another format
```python
# optional parameters are base URL, API version and debug mode
cad_api = CadApi('yourAppKey', 'yourAppSid')
...
try:
    ...
    cad_api.create_saved_image_as(save_as_to_storage_request)

    # download saved image from storage
    saved_file = cad_api.download_file(
    ...
    converted_image = cad_api.create_saved_image_as(
...
# created earlier
api_response = cad_api.create_image_search(CreateImageSearchRequest())
...
cad_api.create_image_features(CreateImageFeaturesRequest(
    search_context_id, image_id=None, images_folder='WorkFolder'))

# wait 'till image features extraction is completed
while cad_api.get_image_search_status(
...
if image_from_storage:
    ...
    results = cad_api.find_similar_images(
...
response = cad_api.find_image_duplicates(
